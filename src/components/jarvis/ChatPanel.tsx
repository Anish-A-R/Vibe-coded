...
export function ChatPanel() {
      ...
      switch (commandResult.type) {
        case 'url': {
          if (commandResult.url) {
            // Try window.open but also provide a clickable link as fallback
            // (window.open may be blocked in sandboxed/iframe environments)
            const newWindow = window.open(commandResult.url, '_blank', 'noopener,noreferrer')
            if (!newWindow || newWindow.closed) {
              // Popup was blocked, show clickable link in chat
              addMessage({
                role: 'assistant',
        ...
        case 'search': {
          if (commandResult.url) {
            const newWindow = window.open(commandResult.url, '_blank', 'noopener,noreferrer')
            if (!newWindow || newWindow.closed) {
              // Popup blocked - show clickable link
              addMessage({
                role: 'assistant',
  ...
  }, [input, personalityMode, soundEnabled, addMessage, clearMessages, clearAllConversations, incrementCommandCount, setAIStatus, addToast, speak, addNotification, voiceLanguage])
    ...
    <div className="flex flex-col h-full glass-panel overflow-hidden relative">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-neon-cyan/10">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Clear Chat button - always visible when there are messages */}
          {messages.length > 0 && (
            <motion.button
